...
while (($OGS_id, $ahrd_function) = each %OGS_ahrd_function){
	...
	if ( exists $apollo_curated_function{$maker_apollo_id} ){
		...
		#Lots of special characters in AHRD desc
		# Dcitr00g03830.1.1	Glycerol-3-phosphate dehydrogenase [NAD(P)+]	IPR011128,PF01210,GO:0016616|GO:0046168
		# Dcitr00g04370.1.1	2-(3-amino-3-carboxypropyl)histidine synthase subunit 1	IPR016435,PF01866
		# Dcitr00g04860.1.1	Glycerol-3-phosphate dehydrogenase [NAD(P)+]
...
sub help {
  ...
  $0:
    ...
    Description:
     ...
     Update functional description generated by AHRD, interproscan etc. with curated functions from Apollo. AHRD description is moved inside (). -RA, -RB... suffixes are added to curated descriptions. Assuming that ALL mRNA have a AHRD functional description or an Unknown protein description
