...
#   --nohits  <0/1>    Include info for Queries with no hits??
...
my ($debug,$rep,$evalcutoff,$qcutoff,$scutoff,$nohits,$out,$cov,$flag,$in,@temp,$result,$hit,$hsp,$i,$j);
...
$qcutoff ||= 0.00000001;
#$nohits ||= 0; if($nohits!=0 && $nohits!=1){system('pod2text',$0), exit 1;}
...
while($result = $in->next_result) {
	...
	#if(!$nohits && ($result->no_hits_found())){next;}
	if(!$nohits && ($result->no_hits_found())){next;}
	...
	if($result->num_hits>0){
		if($flag==0){
			...
			print XLS "NOTE\tCounts for translated blasts (tblastn etc.) will have a mix of amino acid counts and nucleotide counts\n\n";
			$flag=1;
		}
		if($result->query_description){
			...
			print XLS "\nQuery\t",$result->query_name,"\nLength\t",$result->query_length,"\n";
			print XLS "\tE-value\tScore\tLength\tQuery_gaps\t%\tQuery\t%\tHit_gaps\ts%\tHit\t%\tHit name\tDescription\tLength\n";
		}
		...
		while($hit = $result->next_hit ) {
	    	...
	    	if (($hit->significance < $evalcutoff) && ($qaln > $qcutoff) && ($saln > $scutoff)){
	    		print XLS "\t",$hit->significance,"\t",$hit->bits,"\t",$tothsplen,"\t",sprintf("%.3f",$qalng),
	    			"\t",$qhsplen,"\t",sprintf("%.3f",$qaln),"\t",sprintf("%.3f",$salng),"\t",$shsplen,"\t",
	    			sprintf("%.3f",$saln),"\t",$hit->name,"\t",$hit->description,"\t",$hit->length(),"\n"
	    	}
	    	else{
